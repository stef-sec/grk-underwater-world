...
void initShadow(ShadowGPU &shadow, int mapSize) {
    ...
    shadow.uTime = glGetUniformLocation_(shadow.program, "uTime");
...
void beginShadowPass(ShadowGPU &shadow, const Mat4 &lightVP, float time) {
    ...
    glUniform1f_(shadow.uTime, time);
}
