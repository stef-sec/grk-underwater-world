...
float spotConeVolume(vec3 p) {
    ...
    float radialMask = 1.0 - smoothstep(coneRadius * 0.5, coneRadius, radial);
    radialMask = radialMask * radialMask;
...
float moonShaft(vec3 p, vec3 rayDir) {
    ...
    float rayAlign = pow(max(dot(rayDir, L), 0.0), 14.0);
    float surfaceBand = smoothstep(uWaterLevel - 10.0, uWaterLevel - 0.5, p.y);
    float depthFade = 1.0 - smoothstep(uWaterLevel - 18.0, uWaterLevel - 1.5, p.y);
    return surfaceBand * rayAlign * depthFade;
}
...
void main() {
    ...
    for (int i = 0; i < STEPS; ++i) {
        ...
        vec3 moonLit = uMoonColor * moon * 2.2;

        vec3 spotL = normalize(uSpotPos - p);
        float spotPhase = pow(max(dot(-rayDir, spotL), 0.0), 6.0);
        vec3 spotLit = uSpotColor * beam * density * spotPhase * 1.4;

        vec3 sampleLight = moonLit + spotLit;
        sampleLight = min(sampleLight, vec3(0.12));
    ...
    accum = toneMap(accum);
    float alpha = clamp(length(accum) * 1.1, 0.0, 0.95);
